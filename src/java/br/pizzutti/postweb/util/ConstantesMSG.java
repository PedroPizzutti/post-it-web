
package br.pizzutti.postweb.util;

/**
 *
 * @author Pedro
 * 
 * Classe de Mensagens Constantes
 */
public class ConstantesMSG {
    
    public static final String MSG_SUCESSO_CAD_USUARIO = "Novo Usuário Cadastrado!";
    
    public static final String MSG_SUCESSO_LOGOUT = "Obrigado por usar nosssos serviços!";
  
    public static final String MSG_ERRO_CAD_USUARIO = "Problema no Cadastro, Verifique os Campos!";
    
    public static final String MSG_ERRO_CAMPO_OBRIGATORIO = "Campo ? obrigatório!";
    
    public static final String MSG_ERRO_USUARIO_SENHA_INVALIDO = "Usuário/Senha Inválidos!";
    
    public static final String MSG_ERRO_LEMBRETE_VAZIO = "Insira o Lembrete!";

}
